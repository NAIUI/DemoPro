package main

import (
	"chainmaker.org/chainmaker/pb-go/v2/common"
	sdk "chainmaker.org/chainmaker/sdk-go/v2"
	"chainmaker.org/chainmaker/sdk-go/v2/examples"
	"fmt"
	"log"
	"time"
)

const (
	claimContractName        = "TaskContract"
	sdkConfigOrg1Client1Path = "D:\\project\\GolandProjects\\sdk-go\\examples\\sdk_configs\\sdk_config_org1_client1.yml"
)

var tag int64 = 1

func main() {

	fmt.Println("====================== create client ======================")
	client, err := examples.CreateChainClientWithSDKConf(sdkConfigOrg1Client1Path)
	if err != nil {
		log.Fatalln(err)
	}
	startTime := time.Now().UnixNano() / 1e6
	for i := 0; i < 50; i++ {
		go dosome(client)
	}
	for tag != 50 {
	}
	allTime := time.Now().UnixNano()/1e6 - startTime
	fmt.Println(tag)
	fmt.Println(allTime)
	fmt.Println(tag * 1000 / allTime)
}

func dosome(client *sdk.ChainClient) {
	testUserContractClaim(client)
	tag++
}

func testUserContractClaim(client *sdk.ChainClient) {
	//fmt.Println("====================== 调用Task合约UploadTask接口 ======================")
	//result, err := testUserContractClaimInvoke1(client, "UploadTask", true)
	//fmt.Println(result)
	//if err != nil {
	//	log.Fatalln(err)
	//}

	//fmt.Println("====================== 调用Task合约QueryFreeWorkerList接口 ======================")
	//result, err := testUserContractClaimInvoke2(client, "QueryFreeWorkerList", true)
	//fmt.Println(result)
	//if err != nil {
	//	log.Fatalln(err)
	//}

	//fmt.Println("====================== 调用Task合约QueryWorkerList接口 ======================")
	//result, err := testUserContractClaimInvoke3(client, "QueryWorkerList", true)
	//fmt.Println(result)
	//if err != nil {
	//	log.Fatalln(err)
	//}

	fmt.Println("====================== 调用Task合约UpdateWorkerList接口 ======================")
	result, err := testUserContractClaimInvoke4(client, "UpdateWorkerList", true)
	fmt.Println(result)
	if err != nil {
		log.Fatalln(err)
	}
}

func testUserContractClaimInvoke1(client *sdk.ChainClient,
	method string, withSyncResult bool) (string, error) {

	rid := "rid001"
	tid := "tid001"
	reward := "10000"
	t_beg := "20221128"
	t_end := "20221130"
	kvs := []*common.KeyValuePair{
		{
			Key:   "rid",
			Value: []byte(rid),
		},
		{
			Key:   "tid",
			Value: []byte(tid),
		},
		{
			Key:   "reward",
			Value: []byte(reward),
		},
		{
			Key:   "t_beg",
			Value: []byte(t_beg),
		},
		{
			Key:   "t_end",
			Value: []byte(t_end),
		},
	}

	err := invokeUserContract(client, claimContractName, method, "", kvs, withSyncResult)
	if err != nil {
		return "", err
	}

	return rid + " " + tid, nil
}

func testUserContractClaimInvoke2(client *sdk.ChainClient,
	method string, withSyncResult bool) (string, error) {

	rid := "rid001"
	tid := "tid001"
	kvs := []*common.KeyValuePair{
		{
			Key:   "rid",
			Value: []byte(rid),
		},
		{
			Key:   "tid",
			Value: []byte(tid),
		},
	}

	err := invokeUserContract(client, claimContractName, method, "", kvs, withSyncResult)
	if err != nil {
		return "", err
	}

	return rid + " " + tid, nil
}

func testUserContractClaimInvoke3(client *sdk.ChainClient,
	method string, withSyncResult bool) (string, error) {

	var kvs []*common.KeyValuePair

	err := invokeUserContract(client, claimContractName, method, "", kvs, withSyncResult)
	if err != nil {
		return "", err
	}

	return "", nil
}

func testUserContractClaimInvoke4(client *sdk.ChainClient,
	method string, withSyncResult bool) (string, error) {

	workerList := "id001 id002 id003 id004"
	kvs := []*common.KeyValuePair{
		{
			Key:   "workerList",
			Value: []byte(workerList),
		},
	}

	err := invokeUserContract(client, claimContractName, method, "", kvs, withSyncResult)
	if err != nil {
		return "", err
	}

	return workerList, nil
}

func invokeUserContract(client *sdk.ChainClient, contractName, method, txId string,
	kvs []*common.KeyValuePair, withSyncResult bool) error {

	resp, err := client.InvokeContract(contractName, method, txId, kvs, -1, false)
	if err != nil {
		return err
	}

	if resp.Code != common.TxStatusCode_SUCCESS {
		return fmt.Errorf("invoke contract failed, [code:%d]/[msg:%s]\n", resp.Code, resp.Message)
	}

	if !withSyncResult {
		fmt.Printf("invoke contract success, resp: [code:%d]/[msg:%s]/[txId:%s]\n", resp.Code, resp.Message, resp.ContractResult.Result)
	} else {
		fmt.Printf("invoke contract success, resp: [code:%d]/[msg:%s]/[contractResult:%s]\n", resp.Code, resp.Message, resp.ContractResult)
	}

	return nil
}
